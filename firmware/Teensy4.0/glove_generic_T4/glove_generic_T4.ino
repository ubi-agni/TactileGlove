...
//#define NO_ADC
#define ADC_SPI1
#define ADC_AD7490B  // #define ADC_MAX11131
...
//  pinMode (peripheralA4Pin, OUTPUT);  // because no 5th ADC
#endif
...
void read_tactile()
{
    ...
    for (byte SelectADC=0; SelectADC < NUM_ADC; SelectADC++)
    {
     ...
     // Serial.println(AnalogData[taxel_counter]);
      ...
      if (SelectADC == BEND_SENSOR_ADC_ID)
      {
        ...
        #ifdef BEND_SENSORS
          pack_adc_buffer(bend_buf+3*bend_sensor_counter, bend_sensor_counter, AnalogData[taxel_counter + bend_sensor_counter]);  // ? pleas check if this ifdef is okay
        #endif
        bend_sensor_counter++;
      }
      else
  ...
  SP.publish();
  #ifdef BEND_SENSORS
    ret = SP.packData((void *)bend_buf, 2);    // ? pleas check if this if cancelation don't mess up the error handling
  #endif
  if (ret < 0)
