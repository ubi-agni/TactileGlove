...
String serial_string = String("TG_T4_") + String(NUM_ADC,DEC) + String("_") + String(NUM_BEND_SENSORS,DEC) + 
  String("_") + String(ARDUINO,DEC) + String("_") + String(REV,DEC) + String("_") + String(__DATE__).replace(" ", "-") + String("_gloveCEArl_60tax_16bend_imu_T4.ino");
...
const byte slaveA4Pin = 15; //Flexsensors read with ADC MAX11131A T32 A4Pin = 15
...
const byte ledPin = 3; //orange led  = 13(Teensy3.2 and Teensy4.0)
int ledState = LOW;
...
void setup() {
  ...
  Serial.println("before spi_dummy_transfer");
  // needed to get the first time the clock idle HIGH before talking to the BNO085
  spi_dummy_transfer();
  Serial.println("after spi_dummy_transfer");
...
void read_tactile()
{
  ...
  // to ensure the clock is idle high before CS of the IMU is low, we need to do a   transfer with the same settings as the IMU
